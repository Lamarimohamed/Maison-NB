...
// Preset size options for each size type. Admin can still add a custom size.
const LETTER_SIZE_PRESETS = ["XS", "S", "M", "L", "XL", "XXL", "Standard", "One Size"];
const NUMBER_SIZE_PRESETS = ["34", "36", "38", "40", "42", "44", "46", "48"];
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [prodSizeType, setProdSizeType] = useState<"letter" | "number">("letter");
  const [prodSizes, setProdSizes] = useState<string[]>(["S", "M", "L", "XL"]);
  const [prodCustomSizeInput, setProdCustomSizeInput] = useState("");
  ...
  const openNewProductModal = () => {
    ...
    setProdSizeType("letter");
    setProdSizes(["S", "M", "L", "XL"]);
    setProdCustomSizeInput("");
    setProdColors([]);
    setProdColorInput("");
  ...
  const openEditProductModal = (p: Product) => {
    ...
    setProdSizeType(p.size_type || "letter");
  ...
  const handleSizeTypeChange = (type: "letter" | "number") => {
    setProdSizeType(type);
    // Reset selected sizes when switching between number/letter systems
    setProdSizes([]);
  };
  ...
  const handleAddCustomSize = () => {
    const val = prodCustomSizeInput.trim();
                      ...
                      <div className="flex gap-3 bg-[#FAF8F5] p-3 rounded-xl border border-[#B88A44]/15">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-[#786F66]">
                            ...
                            {ord.color && (
                              <>
                                {" · "}اللون: <strong className="font-mono text-[#1F1A17]">{ord.color}</strong>
                              </>
                            )}
                          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 w-full max-w-xl border border-[#B88A44]/30 space-y-4 max-h-[85vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveProduct} className="space-y-3 text-xs">
              ...
              {/* Size Type + Sizes */}
              <div>
                <label className="font-bold text-[#1F1A17]">نوع المقاسات</label>
                <div className="flex gap-2 mt-1.5">
                  <button
                    type="button"
                    onClick={() => handleSizeTypeChange("letter")}
                    className={`flex-1 py-2 rounded-xl border font-bold transition-all ${
                      prodSizeType === "letter"
                        ? "bg-[#1F1A17] text-[#D4AF37] border-[#1F1A17]"
                        : "bg-[#FAF8F5] text-[#1F1A17] border-[#B88A44]/30"
                    }`}
                  >
                    حروف (S, M, L...)
                  </button>
                  <button
                    type="button"
                    onClick={() => handleSizeTypeChange("number")}
                    className={`flex-1 py-2 rounded-xl border font-bold transition-all ${
                      prodSizeType === "number"
                        ? "bg-[#1F1A17] text-[#D4AF37] border-[#1F1A17]"
                        : "bg-[#FAF8F5] text-[#1F1A17] border-[#B88A44]/30"
                ...
                <label className="font-bold text-[#1F1A17] block mt-3">
                  المقاسات المتوفرة <span className="text-red-500">*</span>
                </label>
                <div className="flex flex-wrap gap-2 mt-1.5">
                  {(prodSizeType === "letter" ? LETTER_SIZE_PRESETS : NUMBER_SIZE_PRESETS).map((sz) => (
                    <button
                      type="button"
                      key={sz}
                  ...
                  {/* Any custom sizes added that aren't in the preset list */}
                  {prodSizes
                    .filter(
                      (s) =>
                        !(prodSizeType === "letter" ? LETTER_SIZE_PRESETS : NUMBER_SIZE_PRESETS).includes(s)
                    )
                    .map((sz) => (
                      <button
                        type="button"
                        key={sz}
                        onClick={() => toggleSize(sz)}
                        className="px-3 py-1.5 rounded-lg border text-xs font-bold bg-[#B88A44] text-white border-[#B88A44] flex items-center gap-1"
                      >
                        {sz}
                ...
                <div className="flex gap-2 mt-2">
                  <input
                    type="text"
                    value={prodCustomSizeInput}
                    onChange={(e) => setProdCustomSizeInput(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        handleAddCustomSize();
                    ...
                    placeholder="إضافة مقاس مخصص (مثال: 3XL أو 46)"
              ...
              {/* Colors */}
              <div>
                <label className="font-bold text-[#1F1A17]">الألوان المتوفرة (اختياري)</label>
                <div className="flex gap-2 mt-1.5">
                  <input
                    type="text"
                    ...
                    placeholder="اكتب اسم اللون (مثال: أسود) واضغط إضافة"
                    className="flex-1 p-2.5 border rounded-xl bg-[#FAF8F5]"
                  />
                  <button
                    type="button"
                ...
                {prodColors.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {prodColors.map((c) => (
                      <span
                        key={c}
                        className="px-3 py-1.5 rounded-lg border text-xs font-bold bg-[#FAF8F5] text-[#1F1A17] border-[#B88A44]/30 flex items-center gap-1.5"
                      >
                        {c}
                ...
                <p className="text-[10px] text-[#786F66] mt-1">
                  إذا تركتِ هذا الحقل فارغاً، لن يُطلب من الزبونة اختيار لون عند الطلب.
                </p>
              </div>
